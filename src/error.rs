...
impl Errors {
    // This method is a constructor that creates a new 'Errors' instance.
...
impl<'r> Responder<'r, 'static> for Errors {
    fn respond_to(self, req: &'r Request<'_>) -> response::Result<'static> {
        ...
        status::Custom(Status::UnprocessableEntity, Json(json!({"errors": errors}))).respond_to(req)
    }
}
...
impl FieldValidator {
    pub fn validator<T: Validate>(model: &T) -> Self {
        Self {
    ...
    pub fn extract<T>(&mut self, field_name: &'static str, field: Option<T>) -> Template
    where
        T: Default,
    {
